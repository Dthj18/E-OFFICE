...
async function registrarAccion(subsistema, accion, nombre, correo) {
  try {
    const query = `
      INSERT INTO registro_acciones (subsistema, accion, nombre, correo, timestamp)
      VALUES ($1, $2, $3, $4, $5)
    `;

    await pool.query(query, [subsistema, accion, nombre, correo, new Date()]);
  } catch (error) {
